...
class TambolaTicket {
  ...
  print() {
    ...
    this._entries.map(row => {
      console.log(
        row.map(c => (c == 0 ? '' : c).toString().padStart(2, ' ')).join(' | ')
      );
      console.log('-------------------------------------------');
    });
  }
  ...
  convertToCell() {
    ...
    this._entries.forEach((row, r) => {
      const colCells: { [key: number]: TicketCell } = {};
